...
uint8_t VL6180X_Init(void)
{
    ...
    // 推荐初始化序列（根据VL6180X数据手册）
    VL6180X_WriteReg8(0x0207, 0x01);  // 范围缩放因子
    VL6180X_WriteReg8(0x0208, 0x01);  // 范围缩放因子
    VL6180X_WriteReg8(0x0096, 0x00);  // 内部设置
    VL6180X_WriteReg8(0x0097, 0xFD);  // 内部设置
    VL6180X_WriteReg8(0x00E3, 0x00);  // 内部设置
    VL6180X_WriteReg8(0x00E4, 0x04);  // 内部设置
    VL6180X_WriteReg8(0x00E5, 0x02);  // 内部设置
    VL6180X_WriteReg8(0x00E6, 0x01);  // 内部设置
    VL6180X_WriteReg8(0x00E7, 0x03);  // 内部设置
    VL6180X_WriteReg8(0x00F5, 0x02);  // 内部设置
    VL6180X_WriteReg8(0x00D9, 0x05);  // 内部设置
    VL6180X_WriteReg8(0x00DB, 0xCE);  // 内部设置
    VL6180X_WriteReg8(0x00DC, 0x03);  // 内部设置
    VL6180X_WriteReg8(0x00DD, 0xF8);  // 内部设置
    VL6180X_WriteReg8(0x009F, 0x00);  // 内部设置
    VL6180X_WriteReg8(0x00A3, 0x3C);  // 内部设置
    VL6180X_WriteReg8(0x00B7, 0x00);  // 内部设置
    VL6180X_WriteReg8(0x00BB, 0x3C);  // 内部设置
    VL6180X_WriteReg8(0x00B2, 0x09);  // 内部设置
    VL6180X_WriteReg8(0x00CA, 0x09);  // 内部设置
    VL6180X_WriteReg8(0x0198, 0x01);  // 内部设置
    VL6180X_WriteReg8(0x01B0, 0x17);  // 内部设置
    VL6180X_WriteReg8(0x01AD, 0x00);  // 内部设置
    VL6180X_WriteReg8(0x00FF, 0x05);  // 内部设置
    VL6180X_WriteReg8(0x0100, 0x05);  // 内部设置
    VL6180X_WriteReg8(0x0199, 0x05);  // 内部设置
    VL6180X_WriteReg8(0x01A6, 0x1B);  // 内部设置
    VL6180X_WriteReg8(0x01AC, 0x3E);  // 内部设置
    VL6180X_WriteReg8(0x01A7, 0x1F);  // 内部设置
    VL6180X_WriteReg8(0x0030, 0x00);  // 模式设置
...
uint8_t Soft_I2C_Scan(uint8_t *found_addr)
{
    ...
    {
        Soft_I2C_Start();
        uint8_t ack = Soft_I2C_WriteByte((addr << 1) | 0);
        Soft_I2C_Stop();
        
        if(ack)
        {
            if(found_addr) found_addr[found] = addr;
            found++;
        }
